		...
		public function registrarProductoControlador(){
			...
			if(!in_array($unidad, PRODUCTO_UNIDAD)){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"La PRESENTACIÓN DEL PRODUCTO no es correcta o no la ha seleccionado.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if($stock<=0){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No puedes registrar un producto con stock en 0, debes de agregar al menos una unidad.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
			...
			if($registrar_producto->rowCount()==1){
				$alerta=[
					...
					"texto"=>"El producto ".$nombre." se ha registrado con éxito.",
					"icono"=>"success"
				];
			}else{
				...
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No se pudo registrar el producto, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
		...
		public function eliminarProductoControlador(){
		    ...
		    if($eliminarProducto->rowCount()==1){
		    	...
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido eliminar el producto '".$datos['producto_nombre']."' del sistema, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
		    }
		...
		public function actualizarProductoControlador(){
			...
			if($this->actualizarDatos("producto",$producto_datos_up,$condicion)){
				$alerta=[
					...
					"texto"=>"Los datos del producto '".$datos['producto_nombre']."' se han actualizado correctamente.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido actualizar los datos del producto '".$datos['producto_nombre']."', por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
		...
		public function eliminarFotoProductoControlador(){
    		...
    		if(is_file($img_dir.$datos['producto_foto'])){
		        ...
		        if(!unlink($img_dir.$datos['producto_foto'])){
		            $alerta=[
						"tipo"=>"simple",
						"titulo"=>"HA OCURRIDO UN ERROR",
						"texto"=>"Error al intentar eliminar la foto del producto, por favor inténtelo de nuevo.",
						"icono"=>"error"
					];
					return json_encode($alerta);
		        	exit();
			...
			if($this->actualizarDatos("producto",$producto_datos_up,$condicion)){
				$alerta=[
					...
					"texto"=>"La foto del producto '".$datos['producto_nombre']."' se ha eliminado correctamente.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
		...
		public function actualizarFotoProductoControlador(){
			...
			if($this->actualizarDatos("producto",$producto_datos_up,$condicion)){
				$alerta=[
					...
					"texto"=>"La foto del producto '".$datos['producto_nombre']."' se ha actualizado correctamente.",
					"icono"=>"success"
				];
			}else{
