		...
		public function registrarClienteControlador(){
			...
			if($registrar_cliente->rowCount()==1){
				$alerta=[
					...
					"texto"=>"El cliente ".$nombre." ".$apellido." se ha registrado con éxito.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No se pudo registrar el cliente, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
		...
		public function eliminarClienteControlador(){
			...
			if($id==1){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No podemos eliminar el cliente.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
		    ...
		    if($eliminarCliente->rowCount()==1){
		    	...
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido eliminar el cliente ".$datos['cliente_nombre']." ".$datos['cliente_apellido']." del sistema, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
		    }
		...
		public function actualizarClienteControlador(){
			...
			if($this->actualizarDatos("cliente",$cliente_datos_up,$condicion)){
				$alerta=[
					...
					"texto"=>"Los datos del cliente ".$datos['cliente_nombre']." ".$datos['cliente_apellido']." se han actualizado correctamente.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido actualizar los datos del cliente ".$datos['cliente_nombre']." ".$datos['cliente_apellido'].", por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
