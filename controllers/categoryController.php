		...
		public function registrarCategoriaControlador(){
		    ...
		    if($ubicacion!=""){
		    	if($this->verificarDatos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]{5,150}",$ubicacion)){
			    	$alerta=[
						"tipo"=>"simple",
						"titulo"=>"HA OCURRIDO UN ERROR",
						"texto"=>"La UBICACIÓN no coincide con el formato solicitado.",
						"icono"=>"error"
					];
					return json_encode($alerta);
			        exit();
			...
			if($registrar_categoria->rowCount()==1){
				$alerta=[
					...
					"texto"=>"La categoría ".$nombre." se ha registrado con éxito.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No se pudo registrar la categoría, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
		...
		public function eliminarCategoriaControlador(){
		    ...
		    if($eliminarCategoria->rowCount()==1){
		        ...
		        $alerta=[
					...
					"texto"=>"La categoría ".$datos['categoria_nombre']." ha sido eliminada.",
					"icono"=>"success"
				];
		    	...
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido eliminar la categoría ".$datos['categoria_nombre']." del sistema, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
		    }
		...
		public function actualizarCategoriaControlador(){
			...
			if($this->actualizarDatos("categoria",$categoria_datos_up,$condicion)){
				$alerta=[
					...
					"texto"=>"Los datos de la categoría ".$datos['categoria_nombre']." se han actualizado correctamente.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido actualizar los datos de la categoría ".$datos['categoria_nombre'].", por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
