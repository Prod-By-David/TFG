	...
	class saleController extends mainModel{
        ...
        public function agregarProductoCarritoControlador(){
            ...
            if(empty($_SESSION['datos_producto_venta'][$codigo])){
                ...
                $_SESSION['alerta_producto_agregado']="Se ha agregado <strong>".$campos['producto_nombre']."</strong> a la venta.";
            }else{
                ...
                $_SESSION['alerta_producto_agregado']="Se ha agregado +1 <strong>".$campos['producto_nombre']."</strong> a la venta. Total en carrito: <strong>$detalle_cantidad</strong>.";
            }
        ...
        public function removerProductoCarritoControlador(){
            ...
            if(empty($_SESSION['datos_producto_venta'][$codigo])){
				$alerta=[
					...
					"texto"=>"El producto se ha eliminado de la venta.",
					"icono"=>"success"
				];
				...
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido eliminar el producto, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
            }
            return json_encode($alerta);
        ...
        public function buscarClienteVentaControlador(){
			...
			if($cliente==""){
				...
				<article class="message is-warning mt-4 mb-4">
				    ...
				    <div class="message-body has-text-centered">
				    	<i class="fas fa-exclamation-triangle fa-2x"></i><br>
						Debes de introducir el NÚMERO DE DOCUMENTO, NOMBRE, APELLIDO/S o TELÉFONO del CLIENTE.
				    </div>
				</article>';
				exit();
            }
        ...
        public function agregarClienteVentaControlador(){
			...
			if($check_cliente->rowCount()<=0){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido agregar el cliente debido a un error, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
        ...
        public function removerClienteVentaControlador(){
			...
			if(empty($_SESSION['datos_cliente_venta'])){
				...
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido eliminar el cliente, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];	
			}
			return json_encode($alerta);
        ...
        public function registrarVentaControlador(){
            ...
            if($venta_pagado<$venta_total_final){
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"Está es una venta al contado, el total a pagar por el cliente no puede ser menor al total a pagar.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if($agregar_venta->rowCount()!=1){
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido registrar la venta, por favor inténtelo de nuevo. Código de error: 001.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if($errores_venta_detalle==1){
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido registrar la venta, por favor inténtelo de nuevo. Código de error: 002.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if(!$this->actualizarDatos("caja",$datos_caja_up,$condicion_caja)){
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido registrar la venta, por favor inténtelo de nuevo. Código de error: 003.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            $alerta=[
				...
				"texto"=>"La venta se ha registrado con éxito en el sistema.",
				"icono"=>"success"
			];
			return json_encode($alerta);
	        exit();
		    ...
		    if($check_detalle_venta>0){
		        ...
		        if($eliminarVentaDetalle->rowCount()!=$check_detalle_venta){
		        	$alerta=[
						"tipo"=>"simple",
						"titulo"=>"HA OCURRIDO UN ERROR",
						"texto"=>"No hemos podido eliminar la venta del sistema, por favor inténtelo de nuevo.",
						"icono"=>"error"
					];
					return json_encode($alerta);
			        exit();
