		...
		public function registrarUsuarioControlador(){
		    ...
		    if($this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{3,40}",$apellido)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"El APELLIDO o APELLIDOS no coincide con el formato solicitado.",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
				...
				if(filter_var($email, FILTER_VALIDATE_EMAIL)){
					...
					if($check_email->rowCount()>0){
						$alerta=[
							"tipo"=>"simple",
							"titulo"=>"HA OCURRIDO UN ERROR",
							"texto"=>"El EMAIL que acaba de ingresar ya se encuentra registrado en el sistema, por favor inténtelo de nuevo.",
							"icono"=>"error"
						];
						return json_encode($alerta);
						exit();
            ...
            if($clave1!=$clave2){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"Las contraseñas que acaba de ingresar no coinciden, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
				return json_encode($alerta);
				exit();
    		...
    		if($_FILES['usuario_foto']['name']!="" && $_FILES['usuario_foto']['size']>0){
		        ...
		        if(mime_content_type($_FILES['usuario_foto']['tmp_name'])!="image/jpeg" && mime_content_type($_FILES['usuario_foto']['tmp_name'])!="image/png"){
		        	$alerta=[
						"tipo"=>"simple",
						"titulo"=>"HA OCURRIDO UN ERROR",
						"texto"=>"La imagen que ha seleccionado tiene un formato no permitido.",
						"icono"=>"error"
					];
					return json_encode($alerta);
		            exit();
			...
			if($registrar_usuario->rowCount()==1){
				$alerta=[
					...
					"texto"=>"El usuario ".$nombre." ".$apellido." se ha registrado con éxito.",
					"icono"=>"success"
				];
			}else{
				...
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No se pudo registrar el usuario, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
						...
						<tr class="has-text-centered" >
			                <td colspan="7">
			                    <a href="'.$url.'1/" class="button is-link is-rounded is-small mt-4 mb-4">
			                        Haz click AQUÍ para actualizar la lista
			                    </a>
			                </td>
			            </tr>
					';
		...
		public function eliminarUsuarioControlador(){
		    ...
		    if($eliminarUsuario->rowCount()==1){
		    	...
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido eliminar el usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." del sistema, por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
		    }
		...
		public function actualizarUsuarioControlador(){
            	...
            	if($this->verificarDatos("[a-zA-Z0-9$@.-]{7,100}",$clave1) || $this->verificarDatos("[a-zA-Z0-9$@.-]{7,100}",$clave2)){
			    	...
			    	if($clave1!=$clave2){
						...
						$alerta=[
							"tipo"=>"simple",
							"titulo"=>"HA OCURRIDO UN ERROR",
							"texto"=>"Las nuevas CLAVES que acaba de ingresar no coinciden, por favor inténtelo de nuevo.",
							"icono"=>"error"
						];
						return json_encode($alerta);
						exit();
			...
			if($this->actualizarDatos("usuario",$usuario_datos_up,$condicion)){
				...
				$alerta=[
					...
					"texto"=>"Los datos del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." se han actualizado correctamente.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"HA OCURRIDO UN ERROR",
					"texto"=>"No hemos podido actualizar los datos del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido'].", por favor inténtelo de nuevo.",
					"icono"=>"error"
				];
			}
		...
		public function eliminarFotoUsuarioControlador(){
    		...
    		if(is_file($img_dir.$datos['usuario_foto'])){
		        ...
		        if(!unlink($img_dir.$datos['usuario_foto'])){
		            $alerta=[
						"tipo"=>"simple",
						"titulo"=>"HA OCURRIDO UN ERROR",
						"texto"=>"Error al intentar eliminar la foto del usuario, por favor inténtelo de nuevo.",
						"icono"=>"error"
					];
					return json_encode($alerta);
		        	exit();
			...
			if($this->actualizarDatos("usuario",$usuario_datos_up,$condicion)){
				...
				$alerta=[
					...
					"texto"=>"La foto del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." se ha eliminado correctamente.",
					"icono"=>"success"
				];
			}else{
				$alerta=[
		...
		public function actualizarFotoUsuarioControlador(){
			...
			if($this->actualizarDatos("usuario",$usuario_datos_up,$condicion)){
				...
				$alerta=[
					...
					"texto"=>"La foto del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." se ha actualizado correctamente.",
					"icono"=>"success"
				];
			}else{
